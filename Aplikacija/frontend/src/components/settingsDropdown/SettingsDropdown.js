...
function SettingsDropdown({
  ...
  const handleDelete = () => {
    axios.delete(`${postType}/Delete/${post.id}`).then(() => {
      if (postType === "Comment") {
  ...
  const handlePinn = () => {
    axios.put(`${postType}/SetPinned/${post.id}/${!pinned}`).then((res) => {
  ...
  const handleVerify = () => {
    axios.put(`${postType}/SetVerified/${post.id}/${!verified}`).then((res) => {
  ...
  const handleCancel = () => {
    ...
    axios.patch(`Event/${post.id}/Cancel`).then(() => {
      if (setFeed === null) {
        navigate("/events", { replace: true });
        return;
      }
